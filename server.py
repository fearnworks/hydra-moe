from hydra_moe.inference import get_inference_model, get_base_inference_model, generate_base_output, generate_output, generate_prompt
import inference
...
def get_model_output(moe_chat_history, base_chat_history, stats_chat_history, instruction, method, expertsK, max_tokens):
    ...
    weights = inference.get_weights(instruction, method)
    logger.info(f"Weights: {weights}")
    
    alphas = inference.mult_weights_by_alpha(weights, int(default_config.alphaIn), int(expertsK))
    ...
    output = inference.generate_output(instruction, inference.model, alphas, inference.tokenizer, inference.generation_args, count=max_tokens)
    logger.info(output)
    
    response = output.split("### Assistant:\n")[1].split("</s>")[0].strip()
    moe_chat_history.append((instruction, f"MoE Model ({method}): {response}"))
    
    output_base = generate_base_output(instruction, inference.base_model, alphas, inference.base_tokenizer, inference.generation_args, count=max_tokens)
